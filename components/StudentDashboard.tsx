      ...
      if (activeTab === 'HOME') { 
          ...
          return (
              <div className="space-y-6 pb-24">
                {/* DISCOUNT EVENT BANNER (Always Show if Enabled) */}
                        ...
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-tighter ${discountStatus === 'WAITING' ? 'bg-amber-100 text-amber-700 animate-pulse' : 'bg-red-500 text-white animate-pulse'}`}>
                                {discountStatus === 'WAITING' ? 'Event Starting In' : 'Exclusive Offer'}
                              </span>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                              {discountStatus === 'WAITING' 
                                ? 'Get ready for the biggest discount of the season!' 
                          ...
                          <div className="flex flex-col items-center md:items-end gap-2 w-full md:w-auto">
                            <div className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 font-black text-[10px] uppercase tracking-widest">
                              <Timer size={12} /> {discountStatus === 'WAITING' ? 'Event Starts In' : 'Offer Ends In'}
                            </div>
                            <div className="flex gap-2">
                              {discountTimer ? discountTimer.split(' ').map((part, i) => (
                                <div key={i} className="flex flex-col items-center">
                                  <div className={`px-3 py-2 rounded-xl font-mono text-lg font-black min-w-[3rem] text-center shadow-xl border relative overflow-hidden group/box ${discountStatus === 'WAITING' ? 'bg-amber-600 text-white border-amber-400' : 'bg-slate-900 dark:bg-black text-white border-white/5'}`}>
                                ...
                                <div className="text-slate-400 font-mono text-xs">Calculating...</div>
                              )}
                            </div>
                          </div>
                      </div>
                  ...
                  <div className="bg-gradient-to-r from-pink-50 to-rose-50 p-4 rounded-2xl border border-pink-100 shadow-sm mt-4">
                      ...
                      <div>
                          ...
                          <div className="grid grid-cols-4 gap-3">
                              ...
                              {/* NEW BUTTONS */}
                              {settings?.appMode?.accessTier !== 'FREE_ONLY' && (
                                  <>
