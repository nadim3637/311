...
interface Props {
  ...
  syllabusMode?: 'SCHOOL' | 'COMPETITION' | 'BOTH'; // NEW: Controlled by Admin
...
export const ClassSelection: React.FC<Props> = ({ selectedBoard, allowedClasses, syllabusMode = 'BOTH', onSelect, onBack }) => {
  
  // Filter visible classes based on Syllabus Mode
  const classes = all_classes.filter(c => {
      if (syllabusMode === 'SCHOOL') return c !== 'COMPETITION';
      if (syllabusMode === 'COMPETITION') return c === 'COMPETITION';
      return true; // BOTH
  });
